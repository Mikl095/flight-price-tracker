...
def render_edit_route(r, routes, email_cfg):
    with st.expander("✏️ Éditer ce suivi"):
        with st.form(key=f"dash_form_{r['id']}"):
            ...
            # CLASSE
            class_e = st.selectbox("Classe voyage", ["Eco", "Business", "First"], index=["Eco","Business","First"].index(r.get("travel_class","Eco")))
...
def render_add_tab(routes):
    ...
    with st.form("form_add_new"):
        ...
        travel_class = st.selectbox("Classe voyage", ["Eco","Business","First"], index=0)
