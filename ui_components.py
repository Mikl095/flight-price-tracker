...
from utils.storage import (
    load_routes, save_routes, ensure_route_fields,
    sanitize_dict, count_updates_last_24h, load_email_config, save_email_config, append_log
)
...
def render_dashboard(routes, email_cfg):
    ...
    for r in routes:
        ...
        df_rows.append({
            ...
            "direct": r.get("direct_only", False)
        })
    ...
    for idx, r in enumerate(routes):
        ...
        with cols[0]:
            st.write(
                ...
                f"**Prix cible :** {r.get('target_price')}€\n"
                f"**Email :** {r.get('email') or email_cfg.get('email','—')}\n"
                f"**Bagages min :** {r.get('min_bags',0)}  Vol direct : {r.get('direct_only',False)}"
            )
...
def render_edit_route(r, routes, email_cfg):
    with st.expander("✏️ Éditer ce suivi"):
        with st.form(key=f"dash_form_{r['id']}"):
            ...
            stay_min_e = st.number_input("Séjour min", value=r.get("stay_min",1))
            stay_max_e = st.number_input("Séjour max", value=r.get("stay_max",1))
            target_price_e = st.number_input("Prix cible (€)", value=r.get("target_price",100.0))
            ...
            travel_class_e = st.selectbox("Classe", ["Eco","Business","First"], index=["Eco","Business","First"].index(r.get("travel_class","Eco")))
            min_bags_e = st.number_input("Bagages min", value=r.get("min_bags",0))
        ...
        if submit:
            r.update({
                ...
                "target_price": float(target_price_e),
...
def render_add_tab(routes):
    ...
    with st.form("form_add_new"):
        ...
        stay_min = st.number_input("Séjour min", min_value=1, max_value=365, value=6)
        stay_max = st.number_input("Séjour max", min_value=1, max_value=365, value=10)
        ...
        email_notify = st.text_input("Email pour ce suivi (vide = global)", value="")
        travel_class = st.selectbox("Classe", ["Eco","Business","First"])
        min_bags = st.number_input("Bagages min", min_value=0, max_value=5, value=0)
        direct_only = st.checkbox("Vol direct uniquement", value=False)
        add_submit = st.form_submit_button("Ajouter ce suivi")
    if add_submit:
        for o in [o.strip().upper() for o in origins.split(",") if o.strip()]:
            for d in [d.strip().upper() for d in destinations.split(",") if d.strip()]:
                new = {
                    "id": str(uuid.uuid4()),
                    "origin": o,
                    "destination": d,
...
def render_search_tab(routes):
    st.header("🔎 Recherche & Simulation")
    st.write("Simulation de prix selon origines, destinations, dates et durée de séjour.")
            ...
            results = [
                {
                    ...
                    "return": (start_date + timedelta(days=delta+stay_days)).isoformat(),
                    "stay_days": int(stay_days),
                    "price": random.randint(120,1200)
                }
                for o in origins for d in dests for delta in range(-search_window_days, search_window_days+1)
