...
def render_top_bar(routes, email_cfg):
    ...
    notif_color = "🟢" if email_cfg.get("enabled") else "🔴"
    st.markdown(
        f"<div style='font-size:18px; margin-bottom:15px;'>{notif_color} "
        f"<b>Notifications globales : {'ACTIVÉES' if email_cfg.get('enabled') else 'DÉSACTIVÉES'}</b></div>",
...
def render_add_tab(routes):
    ...
    with st.form("form_add_new"):
        ...
        notif_on = st.checkbox("Activer notifications pour ce suivi", value=True)
...
def render_search_tab(routes):
    ...
    with st.expander("Paramètres de recherche"):
        ...
        run_search = st.button("Lancer la recherche (simulation)")
        ...
        results = [
            {
                ...
                "return": (start_date + timedelta(days=delta + int(stay_days))).isoformat(),
