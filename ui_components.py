...
from utils.storage import (
    load_routes, save_routes, load_email_config, save_email_config,
...
def render_add_tab(routes):
    ...
    with st.form("form_add_new"):
        ...
        email = st.text_input("Email pour ce suivi (vide = global)", value="")
...
def render_edit_route(r, routes, email_cfg):
    with st.expander("✏️ Éditer ce suivi"):
        with st.form(key=f"dash_form_{r['id']}"):
            ...
            target_e = st.number_input("Seuil alerte (€)", value=r.get("target_price",100.0))
...
def render_search_tab(routes):
    ...
    origins_input = st.text_input("Origines (IATA, séparées par ,)", value="PAR")
    destinations_input = st.text_input("Destinations (IATA, séparées par ,)", value="TYO")
    ...
    samples = st.number_input("Échantillons par combinaison", min_value=3, max_value=30, value=8)
        ...
        results = [
            {
                "origin": o,
                "destination": d,
                "departure": (start_date + timedelta(days=random.randint(0,7))).isoformat(),
                "return": (start_date + timedelta(days=stay_days)).isoformat(),
                "stay_days": stay_days,
                "price": random.randint(120,1200)
            }
            for o in origins for d in dests for _ in range(samples)
        ]
