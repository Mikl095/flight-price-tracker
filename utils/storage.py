...
def count_updates_last_24h(route: dict) -> int:
    ...
    for h in route.get("history", []) or []:
        ...
        if not d:
            continue
        try:
            dt = datetime.fromisoformat(str(d)) if isinstance(d, str) else datetime.fromtimestamp(d)
            if dt >= cutoff:
                cnt += 1
        except:
            continue
...
def ensure_route_fields(r: dict):
    keys = ["id","origin","destination","departure","departure_flex_days","return","return_flex_days",
            "return_airport","stay_min","stay_max","target_price","tracking_per_day","notifications",
            "email","min_bags","direct_only","max_stops","avoid_airlines","preferred_airlines",
            "history","last_tracked","stats"]
    defaults = ["","","",None,0,None,0,None,1,1,100.0,1,False,"",0,False,"any",[],[],[],None,{}]
