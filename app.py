# app.py - Flight Price Tracker - Option B (Multi-onglets)
...
with tab_dashboard:
    ...
    if not routes:
        ...
        st.markdown("---")
        # Per-route detailed panels (legacy-style)
...
# ---------------- ADD tab ----------------
with tab_add:
    st.header("➕ Ajouter un suivi (détail)")
    with st.form("form_add"):
        ...
        notifications_on = st.checkbox("Activer notifications pour ce vol", value=True)
...
with tab_config:
    ...
    global_email = st.text_input("Email global (pour notifications)", value=email_cfg.get("email", ""))
    ...
    st.markdown("---")
    st.subheader("Test SendGrid / Email")
    test_email = st.text_input("Adresse pour test", value=global_email or "")
    if st.button("Envoyer email de test"):
        if not test_email:
            ...
            if ok:
                ...
                st.error(f"Erreur d'envoi — status: {status}")
...
with tab_search:
    ...
    st.write("Simule des prix sur une plage et compare origines/destinations.")
    ...
    if "last_search" in st.session_state:
        ...
        st.markdown("Ajouter un résultat au suivi")
        with st.form("add_from_search"):
            ...
            if add_submit:
                ...
                if dep_dt:
                    return_iso = (dep_dt + timedelta(days=row["stay_days"])).date().isoformat()
                else:
                    return_iso = ""
                new = {
                    "id": str(uuid.uuid4()),
